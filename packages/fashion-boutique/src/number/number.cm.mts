...
export class ZNumberComponentModel extends ZCircusComponentModel {
   ...
   *        Enter key.
   * @param repeater -
   *        The key to use to press to repeat the spin
   *        action.  Will be used times - 1 times or 0 if
   *        times is 1 or less.
   *
   * @returns
   *        The input value.
  ...
  private async _spinClick(direction: 1 | -1, times: number, repeater: IZCircusKey): Promise<number | null> {
    ...
    for (let i = 1; i < times; ++i) {
      await spinner.perform(new ZCircusActBuilder().press(repeater).build());
    }
   ...
   * @param repeater -
   *        The key to use to repeat the increment if it is more than
   *        1.
   *
   * @returns
