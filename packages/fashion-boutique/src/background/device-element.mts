...
export interface ZDeviceElement
...
export function ZPropertyDevice<K extends string, T extends HTMLElement>(
  name: string,
  fallback: K,
) {
  return (target: T, propertyKey: string | symbol): void => {
    function get(this: T): Required<IZDeviceValueMap<K>> {
      ...
      const device = new ZDeviceBounds<K>(
        $device?.device?.call($device),
        fallback,
      );
...
export function ZPropertyDeviceWidth<K extends string, T extends HTMLElement>(
  fallback: K,
) {
  return ZPropertyDevice<K, T>("width", fallback);
}

export function ZPropertyDeviceHeight<K extends string, T extends HTMLElement>(
  fallback: K,
) {
