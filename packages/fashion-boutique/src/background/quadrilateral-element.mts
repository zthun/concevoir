...
export function ZPropertyQuadrilateral<V extends string, T extends HTMLElement>(name: string, fallback: V) {
  return function (target: T, propertyKey: string | symbol): void {
    function get(this: T): IZQuadrilateral<V> {
      ...
      const quadrilateral = this.querySelector<ZQuadrilateralElement<V>>(selector);
...
export function ZPropertyQuadrilateralMargin<T extends HTMLElement>(fallback: ZThicknessSize | ZSizeVaried.Fit) {
  return ZPropertyQuadrilateral<ZThicknessSize | ZSizeVaried.Fit, T>('margin', fallback);
}

export function ZPropertyQuadrilateralPadding<T extends HTMLElement>(fallback: ZGapSize) {
  return ZPropertyQuadrilateral<ZGapSize, T>('padding', fallback);
}

export function ZPropertyQuadrilateralEdge<T extends HTMLElement>(fallback: ZThicknessSize) {
  return ZPropertyQuadrilateral<ZThicknessSize, T>('edge', fallback);
}

export function ZPropertyQuadrilateralTrim<T extends HTMLElement>(fallback: Property.BorderStyle) {
