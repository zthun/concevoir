...
import { colorify, IZFashion, ZFashionBuilder, ZHue, ZShade, ZShades, ZSizeFixed } from '@zthun/fashion-palette';
...
export function ZFashionGrid(props: IZFashionGrid) {
  ...
  const renderShade = (hue: ZHue, shade: ZShade) => {
    ...
    return (
      <div
        className={cssJoinDefined('ZFashionGrid-shade', classes.block)}
        key={key}
  ...
  const renderHue = (hue: ZHue) => {
    return (
      <div className={cssJoinDefined('ZFashionGrid-hue', classes.row)} data-hue={hue}>
  ...
  return (
    <div className={cssJoinDefined('ZFashionGrid-root', className)}>
      {renderHue(ZHue.Red)}
      {renderHue(ZHue.Pink)}
