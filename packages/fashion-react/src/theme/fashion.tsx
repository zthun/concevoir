...
export function ZFashionThemeProvider(props: IZFashionThemeProvider) {
  ...
  const _typography = useMemo(() => firstDefined(new ZFashionTypographyBuilder().build(), typography), [typography]);
  ...
  useEffect(() => $tailor.current?.applyTailor(_tailor), [$tailor.current, _tailor]);
  useEffect(() => $typography.current?.applyTypography(_typography), [$typography.current, _typography]);
