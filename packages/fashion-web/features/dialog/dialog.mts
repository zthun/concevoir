...
import { ZFashionRouteBoutique, ZFashionRouteDialog } from '../../src/routes.mjs';
import { ZFashionWorld } from '../fashion-world.mjs';

When('I navigate to the dialog demo page', async function (this: ZFashionWorld<ZDialogPageComponentModel>) {
...
When(
  ...
  async function (this: ZFashionWorld<ZDialogPageComponentModel>, value: ZSideAnchor) {
...
Then(
  ...
  async function (this: ZFashionWorld<ZDialogPageComponentModel>, anchor: 'left' | 'right' | 'top' | 'bottom') {
    const { page } = this.parameters;
    const drawer = await page.drawer();
    await drawer.waitForOpen();
...
Then('the drawer is closed on the dialog demo page', async function (this: ZFashionWorld<ZDialogPageComponentModel>) {
  const { page } = this.parameters;
  const drawer = await page.drawer();
